...
library(maptools)
gpclibPermit()
setwd('~/R-files/nyc-taxi-data-master/analysis/')
source("helpers.R")
...
dados <- cbind(data$long, data$lat)
...
library(ggmap)
mh_map_set = get_googlemap(center = c(-51.228421939611394, -30.02753685571282), zoom = 13, maptype ="satellite")
mapPoints <- ggmap(mh_map_set)
mapPoints
mh_map_set_dois = get_googlemap(center = c(-51.228421939611394, -30.02753685571282), zoom = 13, source="osm")
mapPoints <- ggmap(mh_map_set_dois)
mapPoints


mh_map = get_googlemap(center = c(-51.228421939611394, -30.02753685571282), zoom = 16, style = "feature:transit|visibility:true|elementType:geometry", source="osm")
mh_map = get_googlemap(center = c(-51.228421939611394, -30.02753685571282), zoom = 16, style = "feature:road.local|visibility:off|elementType:geometry|stylers:[{ saturation: -100 },{ visibility:'simplified'}]")
mh_map = get_googlemap(center = c(-51.228421939611394, -30.02753685571282), zoom = 13,  style = "feature:poi|visibility:off")


mapPoints <- ggmap(mh_map)
mapPoints

mapPoints <- ggmap(mh_map) + 
  geom_point(data = dados, aes(x = V1, y = V2), 
             fill = "orange", alpha =0.2, size = 3, shape = 21,stroke = 0) + 
...
library(ggmap)
egy.map <- get_map(location=c(lon=51-2193961139, lat=-30.02753685571282), zoom=6, maptype="terrain", filename="C:/Users/fsmoura/Desktop/docs/mapa/")
save(egy.map, file="C:/Users/fsmoura/Desktop/docs/mapa/")
...
ggmap(mh_map, extent='device') +geom_point(aes(x=V1, y=V2), colour="red", alpha=0.1, size=3, data=dados)



ggmap(mh_map_set_dois, extent='device') +
  geom_density2d(data=dados, aes(x=V1, y=V2), size=.3) +
  stat_density2d(data=dados, aes(x=V1, y=V2,  fill = ..level.., alpha = ..level..), size = 0.01, bins = 16, geom = 'polygon')+
